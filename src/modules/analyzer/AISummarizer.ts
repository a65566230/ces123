...
export class AISummarizer {
    ...
    async summarizeBatch(files, maxConcurrent = 3) {
...
Provide analysis in JSON format with the following structure:
{
  ...
  "recommendations": ["recommendation1"]
    ...
    parseSummaryResponse(response, file) {
        try {
    ...
    basicAnalysis(file) {
        ...
        const keyFunctions = Array.from(functionMatches, m => m[1]).filter((name) => !!name).slice(0, 10);
        const avgLineLength = content.length / lines.length;
        const complexity = avgLineLength > 100 ? 'high' :
