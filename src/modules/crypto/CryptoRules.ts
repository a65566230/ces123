...
export class CryptoRulesManager {
    ...
    loadFromJSON(json) {
        try {
            ...
            if (rules.keywords) {
                rules.keywords.forEach((rule) => this.addKeywordRule(rule));
            }
            if (rules.libraries) {
                rules.libraries.forEach((rule) => this.addLibraryRule(rule));
            }
            if (rules.constants) {
                rules.constants.forEach((rule) => this.addConstantRule(rule));
            }
        }
        catch (error) {
...
//# sourceMappingURL=CryptoRules.js.map