...
export class RuntimeInspector {
    ...
    async enableAsyncStackTraces(maxDepth = 32) {
    ...
    async getCallStack() {
        ...
        try {
            const callStackInfo = {
                callFrames: pausedState.callFrames.map((frame) => ({
    ...
    async getScopeVariables(callFrameId) {
        ...
        const callFrame = pausedState.callFrames.find((frame) => frame.callFrameId === callFrameId);
        if (!callFrame) {
            throw new Error(`Call frame not found: ${callFrameId}. Use getCallStack() to see available frames.`);
        }
        try {
            const scopeVariablesList = [];
    ...
    async getCurrentScopeVariables() {
        const pausedState = this.debuggerManager.getPausedState();
    ...
    async evaluateGlobal(expression) {
        if (!this.enabled || !this.cdpSession) {
            throw new Error('Runtime inspector is not enabled. Call init() or enable() first.');
        }
        if (!expression || expression.trim() === '') {
...
//# sourceMappingURL=RuntimeInspector.js.map