...
export class TokenBudgetManager {
  ...
  getStats(): TokenBudgetStats & { sessionStartTime: number } {
    ...
    for (const call of this.toolCallHistory) {
      toolUsage.set(call.toolName, (toolUsage.get(call.toolName) || 0) + call.estimatedTokens);
    }
    ...
    const topTools = Array.from(toolUsage.entries())
      .map(([tool, tokens]) => ({
        tool,
        tokens,
        percentage: this.currentUsage === 0 ? 0 : Math.round((tokens / this.currentUsage) * 100),
      }))
    ...
    return {
      ...
      topTools,
      warnings: Array.from(this.warnings).map((threshold) => Math.round(threshold * 100)),
      recentCalls: this.toolCallHistory.slice(-20),
      suggestions: this.generateSuggestions(topTools),
  ...
  private generateSuggestions(topTools: Array<{ tool: string; tokens: number; percentage: number }>): string[] {
    ...
    for (const { tool, percentage } of topTools) {
      ...
      if (tool.includes('collect_code')) {
        suggestions.push(`${tool} uses ${percentage}% of the budget. Try smartMode="summary" or "priority".`);
      } else if (tool.includes('get_script_source')) {
        suggestions.push(`${tool} uses ${percentage}% of the budget. Try preview=true first.`);
      } else if (tool.includes('network_get_requests')) {
        suggestions.push(`${tool} uses ${percentage}% of the budget. Reduce the limit or add filters.`);
      } else if (tool.includes('page_evaluate')) {
        suggestions.push(`${tool} uses ${percentage}% of the budget. Query specific properties instead of full objects.`);
      }
    }
