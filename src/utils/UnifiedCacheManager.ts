...
export class UnifiedCacheManager {
  ...
  async getGlobalStats(): Promise<GlobalCacheStats> {
    ...
    const cacheStats: GlobalCacheStats['caches'] = [];
    ...
    return {
      ...
      hitRate,
      caches: cacheStats,
      recommendations: this.generateRecommendations(totalSize, hitRate, cacheStats),
    };
  }
  ...
  private async cleanupLowHitRate(): Promise<void> {
    ...
    for (const cacheStats of stats.caches) {
      if (cacheStats.hitRate === undefined || cacheStats.hitRate >= avgHitRate * this.LOW_HIT_RATE_THRESHOLD) {
        continue;
      }
  ...
  private async cleanupLargeItems(): Promise<void> {
    ...
    const stats = await this.getGlobalStats();
    const sortedCaches = stats.caches.sort((left, right) => right.size - left.size);
  ...
  private generateRecommendations(
    ...
    if (sizeRatio >= 0.9) {
      recommendations.push('CRITICAL: Cache size is above 90%. Run smart_cache_cleanup immediately.');
    } else if (sizeRatio >= 0.7) {
      recommendations.push('WARNING: Cache size is above 70%. Consider cleanup soon.');
    } else if (sizeRatio >= 0.5) {
      recommendations.push('INFO: Cache size is above 50%. Monitor usage.');
    }

    if (hitRate < 0.3) {
      recommendations.push('Low cache hit rate (<30%). Consider adjusting TTL or cache strategy.');
    } else if (hitRate > 0.7) {
      recommendations.push('Good cache hit rate (>70%). Cache is performing well.');
    }

    for (const cache of cacheStats) {
      const cacheRatio = totalSize === 0 ? 0 : cache.size / totalSize;
      if (cacheRatio > 0.5) {
      ...
      if (cache.hitRate !== undefined && cache.hitRate < 0.2) {
        recommendations.push(
          `${cache.name} has a low hit rate (${(cache.hitRate * 100).toFixed(1)}%). Consider disabling or adjusting it.`
        );
      }
    }
